    ...
    if (reklam.some(word => msg.content.includes(word))) {
      try {
        if (!msg.member.permissions.has("BAN_MEMBERS")) {
          msg.delete();
          return msg
            .reply("Hob kardeşşşş dur bakim, napıyon sen?")
            .then(wiskyx => wiskyx.delete({ timeout: 5000 }));
        }
      } catch (err) {
        console.log(err);
...
client.on('message', message => {
    ...
    if (saas === "acik") {
        ...
        if (sa === 'sa' || sa === 'sea' || sa === 'selamunaleyküm' || sa === 'selamun aleyküm')
...
const embed = new discord.MessageEmbed()
.setTitle('Efendim')
.setDescription('Prefiximi arıyorsan **prefixim: .** ')
.setColor('RANDOM')
  if(msg.content == `<@!929693147712340059>`) return msg.channel.send(embed);
});
//------------------etiketprefix son---------------------------//

client.on("ready", async () => {
      let botVoiceChannel = client.channels.cache.get("812229601660764170");
